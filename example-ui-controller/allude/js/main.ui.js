...
// # Mac Addresses
function renderMacAddresses ( ws ) {
    console.log( 'rendering mac addresses' );
    var el = $( '#macAddresses' );
    Restesque.send( ws, Packet.make().uri( '/x24/macs/' ).method( 'GET' ) )
        .success( function ( packet ) {
            console.log( 'Recieved mac addresses', packet.getBody() );
            var body = packet.getBody();
            var str =  Object
                .keys( body )
                .reduce( function ( arr, key ) {
                    arr.push( { mac: key, time: body[ key ] } );
                    return arr;
                }, [] )
                .sort( function ( a, b ) {
                    if ( a.time === b.time ) {
        ...
        .error( function ( err ) {
            console.log( 'Failed to get mac addresses', err );
        });
}
